...
def get_tb_exp_name(args: argparse.Namespace):
    """
    Get the experiment name for tensorboard experiment (Windows/Linux safe).
    """
    # Replace ':' with '-' in the timestamp
    ts = time.strftime('%Y-%b-%d-%H-%M-%S', time.localtime())

    exp_name = ""
    exp_name += "%s_" % args.task.upper()
    exp_name += "%s_" % args.proj_name

    if args.job in ['training', 'resume_training']:
        exp_name += 'TRAIN_'
        exp_name += "MODEL=%s_" % args.model_type.upper()
        exp_name += "DATA=%s_" % args.task_dataset.upper()
        exp_name += "DESC=%s_" % args.description
    ...
    # Remove/replace any invalid characters for Windows
    exp_name = re.sub(r'[<>:"/\\|?*]', '_', exp_name)
...
def get_wandb_exp_name(args: argparse.Namespace):
    ...
    if args.job in ['testing']:
        exp_name += "->%s" % args.test_dataset.upper()
    exp_name += " %s / " % args.model_type.upper()
    exp_name += "%s" % args.training_type.upper()
